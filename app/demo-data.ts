...
export const demoSuites: DemoSuite[] = [
  {
    ...
    products: [
      ...
      },
      {
        ...
        family: "梯度",
        name: "联邦图像训练更新交付",
        tagline: "客户端完成图像模型本地训练后交付梯度或参数增量；接收方不持有原图，也可能从更新信号恢复标签和视觉内容。",
        template: "model",
        inputLabel: "本地训练批次",
        ...
        outputDetail: "同一产品演示覆盖单轮共享梯度和多步参数更新，攻击结果来自 GradInversion、iDLG 与 SME 固定实验。",
        showcase: {
          eyebrow: "GRADIENT & UPDATE EVIDENCE",
          title: "不交付原图，也可能从训练信号看到原图轮廓",
          description: "三张图分别对应单轮梯度、标签泄露适配与多步更新反演；批量大小、网络结构和防御条件会显著改变风险。",
          items: [
            { src: "/demo-assets/model-attack/gradinv-batch1.png", alt: "GradInversion ImageNet batch 1 原图与重建图", label: "GradInversion · ImageNet B=1", metric: "FeatCos 0.901", note: "标签集合恢复 100%，PSNR 8.99 dB；主要保留类别级结构。" },
            { src: "/demo-assets/model-attack/idlg-lfw-batch1.png", alt: "iDLG LFW batch 1 原图与重建图", label: "iDLG · LFW B=1", metric: "标签 100%", note: "适配无池化 CNN + Adam；高数值不可与原论文设置直接比较。" },
            { src: "/demo-assets/model-attack/sme-cifar100.png", alt: "SME CIFAR-100 多步模型更新反演对比", label: "SME · CIFAR-100 E=20", metric: "PSNR 20.56 dB", note: "相对多步 IG 基线 13.85 dB 提升 6.70 dB。" },
          ],
        },
        attacks: [
  ...
  },
  {
    ...
    products: [
      ...
      },
      {
        ...
        outputDetail: "正常产品只返回当前图片的身份预测；攻击结果与图片来自已带入的模型类攻击实验包。",
        previewImage: { src: "/demo-assets/model-attack/face-input-00015.jpg", alt: "CelebA 身份 15 的输入人脸样例" },
        showcase: {
          eyebrow: "MODEL INVERSION EVIDENCE",
          title: "图片预测之后，模型仍可能暴露训练成员与身份原型",
          description: "保留原始指标与真实重建图；不同方法的访问权限和实验协议并不完全相同，因此不做方法排名。",
          items: [
            { src: "/demo-assets/model-attack/face-reconstruction-00015.jpg", alt: "InvAlignment 对身份 15 的重建人脸", label: "InvAlignment · 单身份重建", metric: "Identity 15", note: "黑盒 softmax 后验训练解码器得到的身份原型。" },
            { src: "/demo-assets/model-attack/invalignment-comparison.png", alt: "InvAlignment 二十组原图和重建图对比", label: "InvAlignment · 原图 / 重建对照", metric: "20 组身份", note: "左列为原图，右列为重建结果；可见身份级结构但细节明显模糊。" },
            { src: "/demo-assets/model-attack/revealer-contact-sheet.png", alt: "Revealer 五十个身份类别的重建联系表", label: "Revealer · 50 类总览", metric: "Top-5 68%", note: "100 个身份评估中的 50 类可视化总览；Top-1 为 28%。" },
          ],
        },
        attacks: [
...
const excludedAttacks: Record<string, { name: string; reason: string }> = {
  ...
  "content-vision": { name: "模型能力复制", reason: "当前实验包没有统一查询预算下的替代模型训练协议，因此本 Demo 不执行模型抽取。" },
